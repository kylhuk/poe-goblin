...
class _AffixAllLeagueClickHouse(ClickHouseClient):
    ...
    def execute(self, query: str, settings: Mapping[str, str] | None = None) -> str:  # type: ignore[override]
        del settings
        self.queries.append(query)
        if "ml_item_mod_tokens_v1" in query:
            assert "t.league = ''" not in query
            assert "t.league = 'all'" not in query
            return (
                '{"item_name":"Hubris Circlet","affix_analyzed":"+93 to maximum Life",'
                '"p10":95.0,"median":145.0,"p90":240.0,"items_per_week":0.5,'
                '"items_total":12,"analysis_level":"affix","underpriced_rate":0.25}\n'
            )
        return '{"week_start":"2026-03-02 00:00:00","too_cheap_count":1}\n'
...
def test_pricing_outliers_returns_item_and_affix_summaries_with_weekly_counts() -> None:
    ...
    assert payload["query"]["weeklyAggregation"] == {
        "level": "item_name",
...
def test_pricing_outliers_keeps_affix_rows_when_league_mode_is_all() -> None:
    client = _AffixAllLeagueClickHouse()
    ...
    assert payload["rows"][0]["analysisLevel"] == "affix"
    assert payload["rows"][0]["affixAnalyzed"] == "+93 to maximum Life"
    assert payload["query"]["league"] == "all"


def test_pricing_outliers_keeps_affix_rows_when_league_mode_is_empty() -> None:
    client = _AffixAllLeagueClickHouse()
