...
def test_service_scan_triggers_account_stash_valuation(monkeypatch) -> None:
    cfg = SimpleNamespace(
        enable_account_stash=True,
        rate_limit_max_retries=1,
        rate_limit_backoff_base=0.1,
    ...
    class _DummyHarvester:
        def __init__(self, _client, _clickhouse, _status, **_kwargs):
            return None
    ...
    def _record_scan(_clickhouse, *, league: str, realm: str, account_name: str) -> str:
        called["league"] = league
        called["realm"] = realm
        called["account_name"] = account_name
        return "scan-1"
    ...
    monkeypatch.setattr(service, "StatusReporter", _DummyStatusReporter)
    monkeypatch.setattr(service, "AccountStashHarvester", _DummyHarvester)
    monkeypatch.setattr(service, "run_account_stash_valuation_scan", _record_scan)

    assert service.main(["--once", "--scan"]) == 0
    assert called == {
        "league": "Mirage",
        "realm": "pc",
        "account_name": "qa-exile",
