#!/usr/bin/env python3
"""Backfill mod features for items already in features table."""
...
def get_items_with_empty_features(limit, offset):
    query = f"""
    SELECT f.item_id, groupArray(t.mod_token) as tokens
    FROM poe_trade.ml_item_mod_features_v1 f
    INNER JOIN poe_trade.ml_item_mod_tokens_v1 t 
      ON t.item_id = f.item_id AND t.league = 'Mirage'
    WHERE f.mod_features_json = '{{}}'
    GROUP BY f.item_id
