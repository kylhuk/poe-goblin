...
|---|---|---|---|---|---|---|
| Public-stash liquidity heatmaps (stackable supply + delist cadence) | Measures how fast commodity items rotate so players can liquidate at the right markup | `service:psapi` public-stash stream, ClickHouse bronze/gloss tables for historical cadence, ML influence signals | `service:psapi` | v2 (partial) | ML-augmented scanner recommendations now include `mlInfluenceScore` and `effectiveConfidence` | Depends on additive ClickHouse views/metrics; risk of stale rows until ingestion fully checkpointed (`docs/v2-gap-matrix.md`) |
...
| Ops dashboard drift + rate-limit alarms | Ensures live telemetry/health is trusted so operators can react before profit windows close | ClickHouse `poe_ingest_status`, `v_gold_mart_diagnostics`, telemetry tables; existing deterministic service fields replaced with real queries per remediation notes | none (internal) | v2 (partial) | `v_gold_mart_diagnostics` now provides real freshness/row-count/league-visibility signals | Must keep ClickHouse additions additive-only; needs admin inventory read paths only, no schema drops |
